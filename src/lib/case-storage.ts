 ...
 * 案件のブラウザ保存
 *
 * 複数の案件をLocalStorageに保存し、IDを指定して読み書きします。
 * 案件の本体は案件ごとに別のキー（testimony-board-case:<案件のID>）へ保存し、
 * 一覧（testimony-board-cases）には、案件を開かずに表示できる情報（名前・証言の件数・更新日時）だけを持たせます。
 * 案件ごとに分ける理由は、証言に添えた画像をデータURLで持つため、入力のたびに全案件を書き直すと重くなるからです。
 *
 * 注意:
...
/**
 * 案件の一覧を、更新日時の新しい順で返します。
 * 注意: 一覧は案件の本体から作り直せる補助的なデータのため、読めない場合は例外にせず、空の一覧として扱います。
 */
...
/** 案件を保存し、一覧の項目（名前・証言の件数・更新日時）を更新します。 */
 ...
 * 検証に失敗したデータは、次の保存で失われないよう、案件ごとの退避用のキーへ写してから例外を投げます。
 */
export function loadCase(caseId: Id): Case {
  const stored = localStorage.getItem(caseKeyOf(caseId));
  if (stored === null) throw new Error(`案件が見つかりません: ${caseId}`);
...
/**
 * 案件を1件だけ保存していた頃のデータを、1件目の案件として移行します。
 * 旧データは、移行が誤っていた場合に備えて消しません。同じIDの案件が既にある場合は、二重に移行しないよう何もしません。
