...
/** 開いている案件の名前を表示するだけの、テスト用の画面です。 */
function 案件名の表示() {
  const currentCase = useCurrentCase();
  return <p>案件名: {currentCase.name}</p>;
}

/** 案件を開く枠の中に、テスト用の画面を描画します。 */
...
describe('CaseGate', () => {
  it('保存済みの案件を開いてから、中の画面を表示する', async () => {
  ...
  it('保存されていない案件のIDでは、見つからないことと、一覧への導線を示す', async () => {
    枠の中に描画する('case-unknown');

    const alert = await screen.findByRole('alert');
    expect(alert).toHaveTextContent('案件を開けませんでした');
    expect(screen.getByRole('link', { name: '案件の一覧へ' })).toHaveAttribute('href', '/');
  });
  ...
  it('保存データが検証に失敗した場合は、理由と退避先を示す', async () => {
    ...
    const alert = await screen.findByRole('alert');
    expect(alert).toHaveTextContent('案件データの形式が正しくありません');
    expect(alert).toHaveTextContent('testimony-board-case-backup');
  });

  it('1件だけ保存していた頃のデータを移行してから、その案件を開く', async () => {
