...
describe('PersonDetail', () => {
  ...
  it('メモのメンションでつながったエンティティを、そのエンティティの詳細へのリンクで表示する', () => {
    ...
    useCaseStore.getState().replaceCase(案件);
    render(<PersonDetail personId="person-neighbor" />);

    const 関連 = screen.getByRole('region', { name: '関連するエンティティ' });
    expect(within(関連).getByRole('link', { name: /湖畔の別荘/ })).toHaveAttribute('href', '/places/place-villa');
  });

  it('開いているタブをURLから引き継ぎ、詳細を閉じるリンクと、証言へのリンクに反映する', () => {
    resetMockNavigation('/persons/person-neighbor?tab=map');
    render(<PersonDetail personId="person-neighbor" />);
  ...
  it('どの証言からも参照されていない人物を削除すると、ボードに戻る', async () => {
    ...
    expect(useCaseStore.getState().currentCase.persons.map((person) => person.id)).not.toContain('person-clerk');
  ...
  it('案件に無い人物を開いた場合は、見つからないことを伝え、詳細を閉じられるようにする', () => {
    ...
    expect(screen.getByRole('link', { name: '人物の詳細を閉じる' })).toHaveAttribute('href', '/');
  });
});
...
describe('PlaceDetail', () => {
  ...
  it('この場所を述べている証言を、時系列の並び順で、証言の詳細へのリンクにする', () => {
    ...
    expect(links.map((link) => link.getAttribute('href'))).toEqual([
      '/claims/claim-caretaker',
      '/claims/claim-neighbor',
    ]);
  });
  ...
  it('案件に無い場所を開いた場合は、見つからないことを伝え、詳細を閉じられるようにする', () => {
    ...
    expect(screen.getByRole('link', { name: '場所の詳細を閉じる' })).toHaveAttribute('href', '/');
  });
});
