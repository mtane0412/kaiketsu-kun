...
describe('CaseBoard', () => {
  it('保存済みの案件を復元して時系列のボードを最初に表示し、証言者別に切り替えられる', async () => {
    ...
    expect(mockRouter.replace).toHaveBeenLastCalledWith('/cases/case-lakeside?tab=speaker', { scroll: false });
  });
  ...
  it('証言のカードは、その証言の詳細ページへのリンクになる（編集の導線は詳細ページに一本化している）', async () => {
    ...
    const 隣家の証言 = (await screen.findByText(/明かりがついていて/)).closest('li')!;
    expect(within(隣家の証言).getByRole('link', { name: /を開く$/ })).toHaveAttribute('href', '/cases/case-lakeside/claims/claim-neighbor');
  ...
  it('地図のタブの証言のカードは、戻り先のタブを引き継いだURLへのリンクになる', async () => {
    ...
    expect(within(隣家の証言).getByRole('link', { name: /を開く$/ })).toHaveAttribute('href', '/cases/case-lakeside/claims/claim-neighbor?tab=map');
  });
  ...
  describe('証言の詳細を横に並べる表示（2ペイン）', () => {
    ...
    it('証言を開いたままタブを切り替えると、証言を開いたままのURLに切り替える', async () => {
      ...
      expect(mockRouter.replace).toHaveBeenLastCalledWith('/cases/case-lakeside/claims/claim-neighbor?tab=speaker', { scroll: false });
  ...
  describe('人物・場所の詳細を横に並べる表示（2ペイン）', () => {
    ...
    it('人物を開いたままタブを切り替えると、人物を開いたままのURLに切り替える', async () => {
      ...
      expect(mockRouter.replace).toHaveBeenLastCalledWith('/cases/case-lakeside/persons/person-neighbor?tab=map', { scroll: false });
