 ...
 * すべて主張の集合から導出される表示であり、一次データではありません。
 *
 * 用語:
 ...
 * 「出来事」を表す型はありません。未解決事件では、警察発表の時系列も含めて、語られる出来事はすべて誰かの主張であり、
 * 特定の見立てを基準に置くと比較が偏るためです。ユーザー自身の見立ても、発言者が 'user' の主張として同列に記述します。
 */
export type Claim = {
  id: Id;
  speaker: Speaker;
  ...
  /** 述べられた内容です。 */
  content: string;
  /** この主張が述べられた時点です。述べている内容が起きた時点（when）とは別の時間軸です。 */
  statedAt?: TimeRef;
  /** この主張が言及している人物です。 */
  mentionedPersonIds: Id[];
  /**
   * この主張が述べる内容の日時です。時系列ボード上の位置は決めませんが、
...
export type Case = {
  ...
  /**
   * 時系列ボードの主張の並び順です。要素は 'claim:主張のID' の形のキーです。
   * 載っていない主張は末尾に並べ、存在しない主張のキーは無視します（src/domain/timeline-order.ts）。
   */
